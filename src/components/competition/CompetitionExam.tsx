...
export function CompetitionExam({ questions }: { questions: CompetitionQuestion[] }) {
  ...
  useEffect(() => {
    if (startedAt && remaining === 0) void submit();
    ...
    <section className="mt-6 space-y-4">
      ...
      <div className="rounded-3xl border border-slate-800 bg-[#111214] p-6 shadow-glow">
        ...
        <div className="mt-5 grid gap-3">
          {current.options.map((option) => <button key={option.id} onClick={() => setAnswers((value) => ({ ...value, [current.id]: option.id }))} className={`rounded-2xl border px-4 py-4 text-left ${answers[current.id] === option.id ? 'border-amber-400 bg-amber-500/15' : 'border-slate-700 bg-slate-900/80'}`}><strong className="mr-3">{option.id}</strong>{option.text}</button>)}
        </div>
      </div>
